...
ThreadPool::ThreadPool(size_t num_threads) : running_(true), stop_initiated_(false) {
    if (num_threads == 0) {
        Logger::error("ThreadPool Ctor: Количество потоков не может быть равно 0.");
        throw std::invalid_argument("ThreadPool: Количество потоков не может быть равно 0.");
    }
    Logger::info("ThreadPool Ctor: Создание пула с " + std::to_string(num_threads) + " потоками.");
    workers_.reserve(num_threads);
    for (size_t i = 0; i < num_threads; ++i) {
        try {
            workers_.emplace_back(&ThreadPool::worker_thread_loop, this);
        } catch (const std::system_error& e) {
            Logger::error("ThreadPool Ctor: Не удалось создать рабочий поток #" + std::to_string(i) + ". Ошибка: " + e.what());
            ...
            for (size_t j = 0; j < workers_.size(); ++j) { // workers_.size() будет меньше i
                if (workers_[j].joinable()) {
                    try {
                         ...
                         Logger::error("ThreadPool Ctor: Ошибка при join потока #" + std::to_string(j) + " во время аварийной остановки: " + join_e.what());
                    }
                }
            }
            workers_.clear(); // Очищаем вектор, так как он не полностью инициализирован
            throw std::runtime_error("ThreadPool Ctor: Не удалось инициализировать все рабочие потоки. Пул остановлен.");
        }
    }
    Logger::info("ThreadPool Ctor: Пул потоков успешно запущен. Активных потоков: " + std::to_string(workers_.size()));
}
...
ThreadPool::~ThreadPool() {
    Logger::debug("ThreadPool Dtor: Деструктор вызван.");
    // Убедимся, что stop() был вызван, если пул еще считается активным
    if (running_.load() || !stop_initiated_.load()) { 
        Logger::info("ThreadPool Dtor: Пул не был явно остановлен, вызов stop().");
        stop();
    } else {
        ...
        if(!all_joined && workers_.size() > 0){ // workers_.size() > 0 на случай если stop() очистил workers_
             Logger::warn("ThreadPool Dtor: Обнаружены 'joinable' потоки после вызова stop(). Повторная попытка join (это не должно происходить).");
    ...
    Logger::info("ThreadPool Dtor: Пул потоков уничтожен.");
}
...
void ThreadPool::worker_thread_loop() {
    ...
    Logger::debug("ThreadPool Worker " + thread_id_str + ": Запущен.");
    ...
    while (true) { // Цикл будет прерван return'ами
        { // Начало критической секции для доступа к очереди
            ...
            if (stop_initiated_.load() && tasks_.empty()) {
                Logger::debug("ThreadPool Worker " + thread_id_str + ": Остановка (stop_initiated и очередь пуста).");
            ...
            if (tasks_.empty()) { 
                if (stop_initiated_.load()) { // Если это из-за остановки, выходим
                     Logger::debug("ThreadPool Worker " + thread_id_str + ": Остановка (очередь пуста после пробуждения по stop_initiated).");
                     return;
                }
        ...
        if (task) { // Убедимся, что задача валидна (хотя enqueue не должен добавлять пустые)
            Logger::debug("ThreadPool Worker " + thread_id_str + ": Начинает выполнение задачи.");
            try {
                task();
            } catch (const std::exception& e) {
                Logger::error("ThreadPool Worker " + thread_id_str + ": Поймано std::exception при выполнении задачи: " + e.what());
            } catch (...) {
                Logger::error("ThreadPool Worker " + thread_id_str + ": Поймано неизвестное исключение при выполнении задачи.");
            }
            Logger::debug("ThreadPool Worker " + thread_id_str + ": Завершил выполнение задачи.");
