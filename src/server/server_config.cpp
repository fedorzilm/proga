...
#include "file_utils.h" // Хотя не используется напрямую здесь, может быть связан с логикой путей
#include "logger.h"
#include <fstream>
#include <sstream>
            ...
            try {
                if (key_upper == "PORT") {
                    ...
                    if (pool_s_ul == 0) {
                        ...
                        thread_pool_size = 1;
                    } else if (pool_s_ul > 256) { // Максимальный разумный предел для учебного проекта
                     ...
                     // В предыдущей версии здесь была проверка на value.empty() с return false.
                     // Если tariff_file_path ОБЯЗАТЕЛЕН в конфиге, раскомментируйте проверку.
                     // Если он может быть пустым (и тогда используется default или из CMD), то текущая логика OK.
                     // Для соответствия предыдущей логике, где пустое значение было ошибкой:
                    if (value.empty()){
                    ...
                    // Это поле может быть пустым, тогда используется автоопределение
