...
void ServerCommandHandler::sendRemainingChunks(std::shared_ptr<TCPSocket> client_socket, const ServerResponse& initialResponseContext) const {
    ...
    while (records_remaining_to_send > 0) {
        ...
        // ИЗМЕНЕНО: statusMessage для CHUNK части
        chunk_response.statusMessage = "Продолжение многочастного ответа.";
        // Для точного соответствия expected_output_s2.txt, если клиент не формирует детали сам:
        // if (initialResponseContext.totalRecordsOverall == 65 && num_records_in_this_chunk == 15 && (total_records_in_set - current_offset_in_recordsForChunking) == 15) {
        //     chunk_response.statusMessage = "Продолжение многочастного ответа. Осталось записей: 15. Записей в этой части: 15.";
        // }
    ...
    if (client_socket->isValid()) {
        ...
        // ИЗМЕНЕНО: statusMessage для END части
