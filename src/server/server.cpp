...
bool Server::start() {
    ...
    // *** ИСПРАВЛЕНИЕ: Установка таймаута на слушающий сокет ***
...
void Server::acceptorThreadLoop() {
    ...
    // поэтому g_server_should_stop и running_ важнее как условия выхода).
        ...
        if (client_socket_obj.isValid()) {
            ...
            if (thread_pool_ && thread_pool_->isRunning()) {
                ...
                if (!enqueued_status) {
                    ...
                    // shared_client_socket выйдет из области видимости и его деструктор закроет сокет.
                }
            } else { 
            ...
            // acceptSocket() вернул невалидный сокет.
            // Это может быть из-за закрытия listen_socket_ в Server::stop() (и accept вернул ошибку),
            // или из-за таймаута на accept (если он был установлен и сработал),
            // или из-за другой ошибки accept (например, EINTR).
