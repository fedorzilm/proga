...
#### 2.1.4. Сетевой протокол и передача данных
* **Протокол транспортного уровня:** TCP/IP, выбран для обеспечения надежной доставки сообщений.
* **Протокол прикладного уровня (обмен сообщениями):** Для обмена данными между клиентом и сервером реализован протокол "длина сообщения + данные сообщения":
    * **Префикс длины:** Каждому текстовому сообщению (запрос от клиента или ответ от сервера) предшествует 4-байтовый префикс. Этот префикс представляет собой длину последующих данных сообщения в байтах, закодированную в сетевом порядке байт (big-endian). Для преобразования используются функции `htonl()` при отправке и `ntohl()` при получении.
    * **Данные сообщения:** Сами данные представляют собой текстовую строку в кодировке UTF-8.
    * **Максимальный размер полезной нагрузки:** Для предотвращения чрезмерного использования ресурсов и потенциальных проблем с буферизацией, максимальный размер полезной нагрузки одного сообщения (не включая 4-байтовый префикс длины) ограничен константой `MAX_MESSAGE_PAYLOAD_SIZE` (в текущей реализации 20 МБ).
    * **Реализация:** Класс `TCPSocket` инкапсулирует работу с этим протоколом через методы `sendAllDataWithLengthPrefix()` и `receiveAllDataWithLengthPrefix()`. Это решает проблему передачи и приема сообщений переменной длины и позволяет сторонам корректно определить границы каждого сообщения в TCP-потоке.
* **Проверка каналов обмена и обработка ошибок:**
    * Надежность доставки отдельных байт обеспечивается протоколом TCP.
    * Протокол "длина + данные" обеспечивает корректное выделение полных сообщений из потока.
    * Класс `TCPSocket` включает сохранение кода последней системной ошибки (`errno` или `WSAGetLastError()`) через метод `getLastSocketError()`. Это используется в клиенте и серверной части для более детального логирования и информирования пользователя о причинах сетевых сбоев (например, таймаут, разрыв соединения, невозможность подключения).
    * На стороне клиента установлен настраиваемый таймаут (`client_receive_timeout`, по умолчанию 2 минуты) на операцию ожидания ответа от сервера, реализованный через опции сокета в `TCPSocket::receiveAllDataWithLengthPrefix`.
