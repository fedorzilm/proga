// fedorzilm/proga/proga-master/unit_tests/test_file_utils.cpp
#include "gtest/gtest.h"
#include "file_utils.h" // Включает common_defs.h (и filesystem)
#include "logger.h"
#include <fstream>
#include <filesystem> // Для std::filesystem
#include <iostream>   // Для std::cerr в SetUp/TearDown
...
void create_test_directory_structure(const std::filesystem::path& base_path, const std::string& structure_type) {
    ...
    if (ec) {
        ...
        // Не используем ASSERT_TRUE здесь, так как это вспомогательная функция, вызываемая из тестов.
        // Тесты должны сами проверять успешность создания, если это критично.
        return;
    }
    ...
    // Для getSafeServerFilePath
    std::filesystem::create_directories(base_path / DEFAULT_SERVER_DATA_SUBDIR, ec);
    if (ec) {
...
protected:
    ...
    void SetUp() override {
        // Logger::init(LogLevel::DEBUG, "test_file_utils_run.log");

        char buffer[FILENAME_MAX];
        if (GETCWD(buffer, FILENAME_MAX) != nullptr) {
            original_cwd = buffer;
        } else {
            int getcwd_errno = errno; // Сохраняем errno немедленно
            ...
            try {
                ...
                std::cerr << "FileUtilsTest::SetUp: В качестве original_cwd используется std::filesystem::current_path(): " << original_cwd.string() << std::endl;
        ...
        if (ec_temp_path || system_temp_dir.empty()) {
            ...
            if (!original_cwd.empty() && std::filesystem::exists(original_cwd)) {
                 ...
                 // Крайний случай, если CWD тоже недоступен
                 test_base_dir = "test_temp_fu_"; // Относительный путь
            }
        } else {
    ...
    void TearDown() override {
        if (!original_cwd.empty() && std::filesystem::exists(original_cwd) && std::filesystem::is_directory(original_cwd)) {
            // std::filesystem::current_path(original_cwd, ec); // Если тесты меняют CWD
        }

        if (!test_base_dir.empty()) { 
             // Для отладки можно закомментировать удаление
            // std::cout << "[ЗАЩИТА ОТЛАДКИ FileUtilsTest::TearDown] Планировалось удаление: " << test_base_dir.string() << std::endl;
            std::error_code ec;
    ...
    std::filesystem::path callGetProjectRootPath(const std::filesystem::path& p) {
        if (p.empty()) { 
            std::cerr << "Warning: callGetProjectRootPath получил пустой путь." << std::endl;
    ...
    std::filesystem::path callGetProjectDataPath(const std::string& subdir_file, const std::filesystem::path& exec_path_for_root) {
         if (exec_path_for_root.empty()) {
            std::cerr << "Warning: callGetProjectDataPath получил пустой exec_path_for_root." << std::endl;
...
TEST_F(FileUtilsTest, GetProjectRootPath_FallbackToCWD_WhenNoMarkersOrStructure) {
    ...
    if (ec_set_cwd) {
        std::cerr << "Warning: Could not set CWD for GetProjectRootPath_FallbackToCWD_WhenNoMarkersOrStructure. Test may be unreliable." << std::endl;
...
TEST_F(FileUtilsTest, GetSafeServerFilePath_ValidFilename) {
    ...
    std::string subdir_name = DEFAULT_SERVER_DATA_SUBDIR; 
...
TEST_F(FileUtilsTest, GetSafeServerFilePath_FilenameCleaning) {
    ...
    std::filesystem::path expected_path_ctrl = server_root / subdir_name / "test.db"; // Expect control chars and leading dots to be stripped
    EXPECT_EQ(std::filesystem::weakly_canonical(safe_path_ctrl), std::filesystem::weakly_canonical(expected_path_ctrl));
}

TEST_F(FileUtilsTest, GetSafeServerFilePath_PathTraversalAttempt_ShouldThrowOrCorrect) {
    if (test_base_dir.empty()) GTEST_SKIP() << "test_base_dir не инициализирован, пропускаем тест.";
    std::filesystem::path server_root = test_base_dir / "server_files_root_trav";
    std::filesystem::create_directories(server_root);
    std::string subdir_name = "data_subdir"; 
    ...
    auto path1 = FileUtils::getSafeServerFilePath(server_root.string(), "../secret.txt", subdir_name);
    EXPECT_EQ(std::filesystem::weakly_canonical(path1), std::filesystem::weakly_canonical(server_root / subdir_name / "secret.txt"));

    // Case 2: Test how .filename() component of getSafeServerFilePath handles this.
    // The current implementation of getSafeServerFilePath will take .filename(), resulting in "secret.txt",
    // which will then be correctly sandboxed.
    std::string tricky_filename_internal_dots = std::string("dummy") + std::filesystem::path::preferred_separator + 
                                  ".." + std::filesystem::path::preferred_separator + 
                                  ".." + std::filesystem::path::preferred_separator + "secret.txt";
    ...
    EXPECT_EQ(std::filesystem::weakly_canonical(path_tricky_corrected), std::filesystem::weakly_canonical(server_root / subdir_name / "secret.txt"));
    ...
    abs_outside_file << "absolute";
    abs_outside_file.close();
    
    auto path_abs = FileUtils::getSafeServerFilePath(server_root.string(), absolute_outside_path.string(), subdir_name);
...
TEST_F(FileUtilsTest, GetSafeServerFilePath_EmptyServerRoot_UsesCWDHeuristic) {
    ...
    std::string subdir_name = "test_data_files_cwd_heuristic"; 
    
    std::filesystem::path safe_path;
    std::filesystem::path current_test_cwd = std::filesystem::current_path(); 

    EXPECT_NO_THROW(safe_path = FileUtils::getSafeServerFilePath("", client_filename, subdir_name));
    
    ASSERT_FALSE(safe_path.empty());
    EXPECT_EQ(safe_path.filename().string(), client_filename);
    ASSERT_TRUE(safe_path.has_parent_path());
    EXPECT_EQ(safe_path.parent_path().filename().string(), subdir_name);
    
    EXPECT_TRUE(std::filesystem::exists(safe_path.parent_path())); 
    
    if (std::filesystem::exists(safe_path.parent_path())) {
         if (safe_path.parent_path().has_parent_path() && 
             safe_path.parent_path().parent_path() != system_temp_dir) { 
             std::error_code ec_rm;
             // std::filesystem::remove_all(safe_path.parent_path().parent_path(), ec_rm); // Keep commented for review
            //  std::cout << "[DEBUG FileUtilsTest::GetSafeServerFilePath_EmptyServerRoot_UsesCWDHeuristic TearDown] "
            //            << "Planned removal (COMMENTED): " << safe_path.parent_path().parent_path().string() << std::endl;
         }
    }
}
