// fedorzilm/proga/proga-bro/unit_tests/test_ip_address.cpp
...
TEST_F(IPAddressTest, ComparisonOperators) {
    ...
    EXPECT_TRUE(!(ip_b < ip_a));   // Было причиной ошибки в выводе (строка ~105 или ~118 в зависимости от версии)
    // ip_d <= ip_a  (D <= A) -> true.  !(A < D) -> !(false) -> true.
    EXPECT_TRUE(!(ip_a < ip_d));   
    // ip_b <= ip_a  (B <= A) -> false. !(A < B) -> !(true) -> false.
    EXPECT_FALSE(!(ip_a < ip_b));  // Было причиной ошибки в выводе (строка ~105 или ~112 в зависимости от версии)
    ...
    EXPECT_TRUE(!(ip_a < ip_d));   // Было причиной ошибки в выводе (строка ~116)
    // ip_a >= ip_b  (A >= B) -> false. !(A < B) -> !(true) -> false.
    EXPECT_FALSE(!(ip_a < ip_b));  // Было причиной ошибки в выводе (строка ~112 или ~118)
    // ip_a >= ip_c  (A >= C) -> false. !(A < C) -> !(true) -> false.
    EXPECT_FALSE(!(ip_a < ip_c));  // Было причиной ошибки в выводе (строка ~119)
}
