...
void loopback_server_thread_echo_function(TCPSocket* listen_socket_ptr,
    ...
    try {
        // ИСПРАВЛЕНИЕ: Уведомляем о готовности *перед* блокирующим accept
        {
...
TEST_F(TCPSocketTest, Loopback_ConnectBindListenAcceptSendReceive) {
    ...
    // ИСПРАВЛЕНИЕ: Ожидание готовности сервера перед подключением клиента
    {
    ...
    if (server_was_ready) {
        ...
        if(server_op_success) { // ИСПРАВЛЕНИЕ для -Wdangling-else
...
TEST_F(TCPSocketTest, SendReceive_EmptyString) {
    ...
    std::thread server_thread([&]() { // Лямбда для потока сервера
        try {
            // ИСПРАВЛЕНИЕ: Уведомляем о готовности *перед* accept
...
TEST_F(TCPSocketTest, ReceiveAllData_Timeout) {
    // Logger::init(LogLevel::DEBUG); // Временно для отладки этого теста
    TCPSocket listen_sock;
    ...
    if (server_was_ready) {
        ...
        // char buffer[128]; // Переменная buffer не используется
        bool success_flag = true; // Должен стать false
