...
#include "date.h"       // Предполагается, что common_defs.h включен через date.h
#include <sstream>      // Для тестирования operator<< и operator>>

class DateTest : public ::testing::Test {
protected:
    // Здесь можно разместить общие объекты или настройки для тестов Date
};
...
// Тест IsLeapYear удален, так как isLeap приватный и тестируется косвенно.
// Если бы он был public, тест выглядел бы так:
/*
TEST_F(DateTest, IsLeapYear_IfPublic) {
    Date d; // Нужен экземпляр для вызова нестатического isLeap
    EXPECT_TRUE(d.isLeap(2000));
    EXPECT_TRUE(d.isLeap(2024));
    EXPECT_FALSE(d.isLeap(1900));
    EXPECT_FALSE(d.isLeap(2023));
    EXPECT_FALSE(d.isLeap(2100));
}
*/
